...
def addcapitalfeatures(feature_vector, capitalcount):
	feature_vector.append(capitalcount)
	return feature_vector



def addpunctfeatures(feature_vector, puncts):
	feature_vector.append(puncts)
	return feature_vector



def addlengthfeatures(feature_vector, length):
	if(length>10):
		feature_vector.append(1)
	else:
		feature_vector.append(0)
...
def addidiomfeatures(feature_vector, idiom):
	feature_vector.append(idiom)
	return feature_vector

def buildtruthfeaturevector(key, tweet):
	global char_n_grams_index, word_n_grams_index, truth_top_hashtags, truth_top_hi_tokens, truth_top_en_tokens, truth_top_rest_tokens

	emoticons, hashtags, mentions, urls, char_n_grams, word_n_grams, idiom, capitalcount, puncts= pp.preprocess(key, tweet)
	...
	truth_feature_vector = addcapitalfeatures(truth_feature_vector, capitalcount)
	truth_feature_vector = addidiomfeatures(truth_feature_vector,idiom)
	truth_feature_vector = addpunctfeatures(truth_feature_vector,puncts)
	#truth_feature_vector = addlengthfeatures(truth_feature_vector,length)

	#print("length of feature vector", len(truth_feature_vector))
